    ...
    for iteration_number in range(100):
        ...
        for i in graph.nodes():
            ...
            for j in graph.neighbors(i):
                neighbors_sum = sum(graph.edge_weight((j, k)) * graph.node_weight[k] for k in graph.neighbors(j))
                rank += damping * scores[j] * graph.edge_weight((j, i)) * graph.node_weight[j]/ neighbors_sum
